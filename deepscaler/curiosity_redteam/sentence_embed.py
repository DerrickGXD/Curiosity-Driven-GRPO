...
class CosineSentenceEmbeddingReward(object):
    ...
    def __init__(self, device: str = "cuda", n_samples: int = -1, impl: str ="huggingface", batch_size: int = 32, is_reasoning_pattern : bool = False):
        self.device = device
        self.n_samples = n_samples
        self.impl = impl
        self.batch_size = batch_size
        self.is_reasoning_pattern = is_reasoning_pattern
        # self.url = "http://10.96.176.212:5000/data"
    ...
    def _compuate_embeddings(self, sentences: List[str]) -> torch.Tensor:
        with torch.no_grad():
            if self.impl == "huggingface":
                ...
                # sentence_embeddings = torch.tensor(response.json()["message"])
    ...
    def non_reasoning_pattern_append_reference(self, prompt, ref):
        ...
        # import ipdb; ipdb.set_trace()
        if prompt not in self.sentence_embeddings:
            self.sentence_embeddings[prompt] = sentence_embeddings
        else:
            self.sentence_embeddings[prompt] = torch.cat([self.sentence_embeddings[prompt], sentence_embeddings], dim=0)
    ...
    def reasoning_pattern_call(self, prompt, hypotheses, reasoning_pattern):
        ...
        if self.n_samples > 0:
            sample_size = min(len(self.sentence_embeddings[prompt][reasoning_pattern]), self.n_samples)
            sample_indices = random.sample(range(self.sentence_embeddings[prompt][reasoning_pattern].shape[0]), k=sample_size)
            ref_sentence_embeddings = self.sentence_embeddings[prompt][reasoning_pattern][sample_indices, :]
        else:
    ...
    def non_reasoning_pattern_call(self, prompt, hypotheses):
        ...
        if self.n_samples > 0:
            sample_size = min(len(self.sentence_embeddings[prompt]), self.n_samples)
            sample_indices = random.sample(range(self.sentence_embeddings[prompt].shape[0]), k=sample_size)
            ref_sentence_embeddings = self.sentence_embeddings[prompt][sample_indices, :]
        else:

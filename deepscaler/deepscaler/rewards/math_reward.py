...
class RewardMathFn(RewardFn):
    ...
    def __call__(self, input: RewardInput) -> RewardOutput:
        ...
        if THOUGHT_DELIMITER_START in model_response and THOUGHT_DELIMITER_END in model_response:
            ...
            return RewardOutput(reward=self.config.format_error_reward, is_correct=False, extract_answer=None)
        
        model_answer = extract_answer(model_solution)
        if model_answer is None:
            return RewardOutput(reward=self.config.format_error_reward, is_correct=False, extract_answer=model_answer)

        # Process the ground truth(s)
        ground_truths = input.ground_truth.get("answer", None)
        if ground_truths is None:
            return RewardOutput(reward=self.config.unk_error_reward, is_correct=False, extract_answer=model_answer)
        ...
        for ground_truth in processed_ground_truths:
            ...
            if is_correct:
                return RewardOutput(reward=self.config.correct_reward, is_correct=True, extract_answer=model_answer)
        ...
        return RewardOutput(reward=self.config.incorrect_reward, is_correct=False, extract_answer=model_answer)
...
def deepscaler_reward_fn(solution_str: str, ground_truth: Union[str, List[str]], enable_llm = False):
    ...
    return reward_response.is_correct, reward_response.extract_answer

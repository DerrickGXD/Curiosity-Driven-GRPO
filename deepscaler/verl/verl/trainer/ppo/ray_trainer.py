...
def compute_data_metrics(batch, use_critic=True):
    ...
    metrics = {
        ...
        # rm reward
        'critic/rm/mean':
            torch.mean(sequence_rm).detach().item(),
        'critic/rm/max':
            torch.max(sequence_rm).detach().item(),
        'critic/rm/min':
            torch.min(sequence_rm).detach().item(),
        # format_correct reward
        'critic/format_correct/mean':
            torch.mean(sequence_format_correct).detach().item(),
        'critic/format_correct/max':
        ...
        'critic/reasoning_pattern_description_reward/max':
            torch.max(sequence_reasoning_pattern_description_reward).detach().item(),
        'critic/reasoning_pattern_description_reward/min':
            torch.min(sequence_reasoning_pattern_description_reward).detach().item(),
        # reasoning_pattern_description_reward reward
        'critic/calculation_reward_tensor/mean':
            torch.mean(sequence_calculation_reward_tensor).detach().item(),
        'critic/calculation_reward_tensor/max':

...
class RewardManager():
    ...
    def __call__(self, data: DataProto, epoch=None):
        ...
        reasoning_pattern_exists_tensor = torch.zeros(len(data), dtype=torch.float32)
        ...
        def process_item(args):
            ...
            if format_correct:
                ...
                # TODO: use some methods to check validity of reasoning pattern
                ...
                if(reasoning_pattern_exists):
                    ...
                    section_rewards["Reasoning Pattern Description"] = self.reasoning_pattern_description_reward_module(prompt_str, [extracted_data["Reasoning Pattern Description"]], reasoning_pattern)
                else:
                    section_rewards["Reasoning Pattern Description"] = 0.0

                section_rewards["Calculation"] = self.calculation_reward_module(prompt_str, [extracted_data["Calculation"]])
                ...
                if(rm_score==0.0):
                    # penalise novelty only when answer is not correct
                        ...
                        for i in range(last_position, len(tokens) - section_len + 1):
                            if tokens[i:i + section_len] == section_tokens:
                                ...
                                if(key=="Reasoning Pattern"):
                                    ...
                                    if(key=="Reasoning Pattern Description"):
                ...
                reasoning_pattern_description_reward = section_rewards["Reasoning Pattern Description"]
                calculation_reward = section_rewards["Calculation"]
                extract_answer = extracted_data["Answer"]
            else:
                reward_tokens[-1] = -1.0 * self.config.curiosity.format_error_reward_coef

                rm_score = 0.0
                reasoning_pattern_exists = False
                reasoning_pattern_description_reward = 0.0
                calculation_reward = 0.0
                extract_answer = "None"
                extracted_data = None
            ...
            score_dict = {
                            ...
                            "reasoning_pattern_exists": reasoning_pattern_exists,
        ...
        for i, reward_tokens, valid_response_length, score_dict, prompt_str, response_str, extract_answer, extracted_data, reasoning_pattern in results:
            ...
            reasoning_pattern_exists_tensor[i] = score_dict["reasoning_pattern_exists"]
            ...
            if self.num_examine == 0 and extracted_data!=None:
                if reasoning_pattern != None and extracted_data["Reasoning Pattern Description"] != None:
                    self.reasoning_pattern_description_reward_module.append_reference(prompt_str, extracted_data["Reasoning Pattern Description"])
        ...
        return reward_tensor, rm_tensor, format_correct_tensor, reasoning_pattern_exists_tensor, reasoning_pattern_description_reward_tensor, calculation_reward_tensor
...
def main_task(config):
    ...
    filename_parameters = f"bs_{config.data.train_batch_size}_rollout_{config.actor_rollout_ref.rollout.n}_reasoningpatterndescription_{config.reward_model.curiosity.reasoning_pattern_description_penalty_type}_{config.reward_model.curiosity.reasoning_pattern_description_reward_coef}_calculation_{config.reward_model.curiosity.calculation_penalty_type}_{config.reward_model.curiosity.calculation_reward_coef}_formaterror_{config.reward_model.curiosity.format_error_reward_coef}_answer_{config.reward_model.curiosity.answer_reward_coef}"

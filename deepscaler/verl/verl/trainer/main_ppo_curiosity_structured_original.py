...
class RewardManager():
    ...
    def __call__(self, data: DataProto, epoch=None):
        ...
        def process_item(args):
            ...
            reward_tokens[-1] += rm_score

            reasoning_pattern_reward = 0.0
            ...
            return i, reward_tokens, valid_response_length, score_dict, prompt_str, response_str, extract_answer, extracted_data
...
@hydra.main(config_path='config', config_name='ppo_trainer_curiosity_original', version_base=None)
def main(config):
...
def main_task(config):
    ...
    filename_parameters = f"qwen7b_bs_{config.data.train_batch_size}_rollout_{config.actor_rollout_ref.rollout.n}_original"
